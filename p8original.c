...
{
  int x;
  printf("How any fractions you wish to add:- ");
  scanf("%d",&x);
  return x;
}
Fraction input_fraction(int n,Fraction *a,Fraction *b)
{
  int i;
  for(i=0;i<n;i++)
  {
    printf("Enter %d numerator:- ",i+1);
    scanf("%d",&a[i].num);
    printf("Enter %d denominator:- ",i+1);
    scanf("%d",&b[i].den);
  }
}
Fraction add_n_fractions(int n,Fraction *a,Fraction *b)
{
  Fraction sum;
  int i;
  for(i=0;i<n;i++)
  {
    sum.num=(a[i].num*a[i+1].den)+(a[i+1].num*a[i].den);
    sum.den=a[i].den*a[i+1].den;
  }
}
void output(int n,Fraction sum)
{
...
int main()
{
  ...
  n=input_n();
  Fraction a[n],b[n];
  input_fraction(n,&a[n],&b[n]);
  Fraction sum=add_n_fractions(n,&a[n],&b[n]);
  output(n,sum);
  return 0;
}
